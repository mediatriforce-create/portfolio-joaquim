...
import { Terminal, Box, ShieldCheck, Zap } from "lucide-react";
...
function MobileHero() {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center px-6 py-20 overflow-hidden">
      ...
      {/* Role */}
      <FadeIn delay={0.2} className="mt-4 mb-4">
        <span className="text-[10px] font-mono tracking-[0.2em] text-[#00ff66] uppercase">
...
function MobileAbout() {
  ...
  return (
    <section className="relative px-5 py-20 border-t border-white/10">
      {/* Avatar */}
      ...
      <FadeIn className="mt-12">
        <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/10 relative overflow-hidden">
          ...
          <p className="text-lg font-bold text-white leading-tight mb-4 relative z-10">
    ...
    <section className="px-5 py-16 border-t border-white/5">
      ...
      <div className="grid grid-cols-5 gap-3">
        {TOOLS.map((tool, i) => {
          ...
          return (
            <FadeIn key={tool.name} delay={i * 0.02} className="flex flex-col items-center gap-1.5">
              <div
                className="flex items-center justify-center w-10 h-10 rounded-lg bg-white/[0.04] border border-white/8"
                style={{ boxShadow: `0 0 8px ${tool.color}10` }}
              >
                <Icon className="w-4 h-4" color={tool.color} />
              </div>
              <span className="font-mono text-[6px] text-neutral-600 uppercase tracking-wide text-center leading-tight">
                {tool.name}
              </span>
            </FadeIn>
          );
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-full max-h-[85vh] overflow-y-auto overscroll-contain rounded-2xl bg-neutral-900 border border-neutral-800 shadow-2xl"
        ...
        {PROJECTS.map((project, i) => (
          <FadeIn key={project.id} delay={i * 0.1}>
            <button
              ...
              className="w-full text-left rounded-2xl overflow-hidden bg-neutral-900/50 border border-white/[0.06] active:scale-[0.98] transition-transform"
            >
              {/* Image */}
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        ...
        <FadeIn delay={0.4} className="flex flex-col items-center">
          <button
            ...
            className="group relative inline-flex items-center gap-2 rounded-full overflow-hidden cursor-pointer mb-6"
          >
            ...
            {showLinks && (
              ...
              >
                <motion.a
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0 }}
                  className="flex items-center gap-3 px-5 py-4 rounded-xl border border-white/10 bg-white/[0.03] active:bg-[#00ff66]/[0.05]"
                >
