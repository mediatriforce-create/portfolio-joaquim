  ...
  return (
    <section className="relative px-5 py-16 border-t border-white/10">
      ...
      {chapters.map((ch) => {
        const Icon = ch.icon;
        return (
          <div key={ch.id} className="mb-8 pl-4 border-l-2 border-[#00ff66]/30">
      ...
      {/* Manifesto */}
      <div className="mt-4 p-5 rounded-2xl bg-white/[0.02] border border-white/10 relative overflow-hidden">
...
function MobileProjectModal({ project, onClose }: { project: Project; onClose: () => void }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex items-end justify-center"
      onClick={onClose}
    >
      <motion.div
        initial={{ y: "100%" }}
        ...
        className="relative w-full max-h-[90vh] overflow-y-auto overscroll-contain rounded-t-2xl bg-neutral-900 border-t border-neutral-700 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Grab bar */}
        <div className="sticky top-0 z-20 flex justify-center pt-3 pb-2 bg-neutral-900">
        ...
        {/* Close */}
        <button onClick={onClose} className="absolute top-3 right-3 w-8 h-8 rounded-full bg-black/60 border border-white/10 flex items-center justify-center z-30">
